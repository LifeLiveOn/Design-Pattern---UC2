...
order_value > 500 -> SetPriorityShipping()'''
    ...
    test_data = {"age": 25, "location": "NY", "weight": 4.5,
                 "distance": 80, "order_value": 600, "total": 100}
    print(f"\nTest Data: {test_data}")
...
def test_action_flexibility():
    ...
    from action import create_action_command, Action

    # Test creating various action commands as strings
    cmd1 = create_action_command("ApplyDiscount(10)")
    print(f"Discount command: {cmd1}")
    assert cmd1.execute() == "ApplyDiscount(10)"

    cmd2 = create_action_command("CalculateFedExShipping(5.5, 100)")
    print(f"FedEx command: {cmd2}")
    assert cmd2.execute() == "CalculateFedExShipping(5.5, 100)"
    ...
    # Test custom action names work too
    cmd6 = create_action_command("additional customs documentation")
    print(f"Custom command: {cmd6}")
    assert isinstance(cmd6, Action)
